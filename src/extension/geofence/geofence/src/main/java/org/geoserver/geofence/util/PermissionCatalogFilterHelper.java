...
/**
 * Helper class that translates a {@link PermsResult} from GeoFence into a GeoServer catalog
 * pre-filter ({@link Filter}).
 *
 * <p>The CQL filter inside {@link PermsResult} uses the property names {@code workspace} and
 * {@code layer} as produced by the GeoFence rule engine. This helper renames those properties to
 * the appropriate GeoServer catalog property paths depending on the target {@link CatalogInfo}
 * subtype.
 *
 * @author etj
 */
public class PermissionCatalogFilterHelper {
     ...
     *
     * <p>The filter restricts catalog listings to objects the user has at least read access to.
     * Property names in the result are tailored to the target {@link CatalogInfo} subtype.
     *
     * <p>For {@link WorkspaceInfo}, visibility is derived from the accessible-resources set (since
     * the CQL layer conditions are not meaningful at workspace level). For all layer-level types
     * ({@link LayerInfo}, {@link LayerGroupInfo}, {@link ResourceInfo}) the CQL filter provided by
     * GeoFence is parsed and its {@code workspace} / {@code layer} property names are remapped to
     * the correct catalog property paths.
     *
    ...
    public Filter buildCatalogFilter(PermsResult permsResult, Class<? extends CatalogInfo> clazz) {
        ...
        if (LayerInfo.class.isAssignableFrom(clazz)) {
            ...
            LOGGER.log(
                    Level.WARNING,
                    "Unhandled catalog type for GeoFence security pre-filter: {0}",
                    clazz.getName());
    ...
    private Filter buildWorkspaceFilter(PermsResult permsResult) {
        ...
        if (resources.contains("*:*")
                || resources.stream().anyMatch(r -> r.startsWith("*:"))) {
            return Filter.INCLUDE;
        }
    ...
    /**
     * Parses the CQL filter string and renames {@code workspace} / {@code layer} property
     * references to the target property paths.
     */
    private Filter parseCqlAndRenameProperties(
            String cqlFilter, String newWsProp, String newLayerProp) {
        Filter filter;
        try {
            ...
            LOGGER.log(
                    Level.WARNING,
                    "Failed to parse PermsResult CQL filter '" + cqlFilter + "', denying access",
                    e);
            return Filter.EXCLUDE;
        }
        ...
        PropertyNameRemapper remapper =
                new PropertyNameRemapper(
                        Map.of(GF_WORKSPACE, newWsProp, GF_LAYER, newLayerProp));
        return (Filter) filter.accept(remapper, null);
    }

    /**
     * A {@link DuplicatingFilterVisitor} that renames {@link PropertyName} nodes according to a
     * given mapping.
     */
